...
def request_callback(request):
    if request.url.endswith("excel"):
        with open(
            "tap_sharepointsites/tests/configuration/sample_excel.xlsx", "rb"
        ) as file:
        ...
        with open(
            "tap_sharepointsites/tests/configuration/sample.csv", "r", encoding="utf-8"
        ) as file:
            csv_data = file.read()
...
def drive_id_response():
    js = json.loads(
        open(
            "tap_sharepointsites/tests/configuration/drive_id_response.json", "r"
        ).read()
    )
    return js
...
def list_files_response():
    js = json.loads(
        open(
            "tap_sharepointsites/tests/configuration/file_info_response.json", "r"
        ).read()
    )
    return js
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "sample.csv",
            True,
            ["id_column", "first_name", "last_name", "best_invisible_color"],
        ),
        (
            "csv",
            "sample.csv",
            False,
            ["ID Column", "First Name", "Last Name", "Best (invisible) color"],
        ),
        (
            "excel",
            "sample_excel.xlsx",
...
@responses.activate
def test_sync_excel_file_2(
    mock_az_default_identity,
    capsys,
    filetype,
    filename,
    clean_columns,
    ...
    responses.add_callback(
        responses.GET,
        re.compile(
            r"https://m365x214355\.sharepoint\.com/sites/SingerTests/_layouts/15/download\.aspx\?UniqueId=[^&]+"
        ),
    ...
    responses.add(
        responses.GET, f"{custom_config['api_url']}drive", json=drive_id_response()
    )
    ...
    if clean_columns:
        ...
        last_names = [
            row["record"]["Last Name"]
            for row in records
            if row["record"]["Last Name"] != ""
            and row["record"]["Last Name"] is not None
        ]
