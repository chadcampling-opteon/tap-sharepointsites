...
def test_stuff(mock_az_default_identity, capsys):
    ...
    for part in stdout_parts:
        ...
        if part_json["type"] == "SCHEMA":
            ...
            assert part_json["schema"]["properties"]["createdDateTime"]["type"] == [
                "string",
                "null",
            ]
            assert part_json["schema"]["properties"]["eTag"]["type"] == [
                "string",
                "null",
            ]
            assert part_json["schema"]["properties"]["id"]["type"] == ["string", "null"]
            assert part_json["schema"]["properties"]["lastModifiedDateTime"][
                "type"
            ] == ["string", "null"]
            assert part_json["schema"]["properties"]["webUrl"]["type"] == [
                "string",
                "null",
            ]
            assert part_json["schema"]["properties"]["createdBy"]["type"] == [
                "object",
                "null",
            ]
            assert part_json["schema"]["properties"]["lastModifiedBy"]["type"] == [
                "object",
                "null",
            ]
            assert part_json["schema"]["properties"]["parentReference"]["type"] == [
                "object",
                "null",
            ]
            assert part_json["schema"]["properties"]["contentType"]["type"] == [
                "object",
                "null",
            ]
        if part_json["type"] == "RECORD":
            ...
            assert (
                part_json["record"]["eTag"] == "c8cf0cd9-826e-4295-8c52-e2c201739dc5,13"
            )
            assert part_json["record"]["lastModifiedDateTime"] == "2017-09-02T01:45:12Z"
            assert (
                part_json["record"]["webUrl"]
                == "https://m365x214355.sharepoint.com/Lists/Office%20365%20Demos/6_.000"
            )
            assert (
                part_json["record"]["createdBy"]["user"]["email"]
                == "provisioninguser1@m365x214355.onmicrosoft.com"
            )
